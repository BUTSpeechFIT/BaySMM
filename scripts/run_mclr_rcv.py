...
def create_file_suffix(args):
    sfx = f"{args.clf}_R{args.R}_{args.in_lang}_{args.ntrain}_{args.split_ix}"
    return sfx


def load_feats_and_labels(emb_dir, in_lang, config, set_name, args):
    ...
    dim_read = "half" if args.clf == "mclr" else "full"

    if L2I:
        labels = rcv_utils.load_labels(labels_f, L2I)
    else:
    ...
    if feats_f:
        feats = rcv_utils.load_feats(feats_f, dim=dim_read)
...
def classify_dev_test_data(
    model, emb_dir, config, langs, args, scores_dict, proba_dict
):
    ...
    for i, set_name in enumerate(["dev", "test"]):
        ...
        for j, lang in enumerate(langs):
            ...
            x_test, y_test = load_feats_and_labels(
                emb_dir, lang, config, set_name, args
            )

            if x_test is None:
                continue

            rcv_utils.check_feats_and_labels(x_test, y_test)
...
def main():
    ...
    if not args.out_dir:
        args.out_dir = os.path.realpath(args.emb_dir) + "/../results_mclr/"
    ...
    x_dev, y_dev = load_feats_and_labels(emb_dir, args.in_lang, config, "dev", args)
    ...
    scores_dict, proba_dict = classify_dev_test_data(
        best_model, args.emb_dir, config, langs, args, scores_dict, proba_dict
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "clf",
        choices=["mclr", "mclru", "mclru-0"],
        help="multi-class logistic regression (with uncertainty)",
    )
    parser.add_argument(
        "cfg_file", help="path to config json file corresponding to the model that is used to extract the embeddings."
    )
    ...
    parser.add_argument("-emb_dir", default="", help="path to dir with embeddings (by default it will be taken from the config file)")
