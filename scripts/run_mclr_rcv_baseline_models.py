...
def create_file_suffix(args):
    sfx = f"{args.clf}_{args.in_lang}_{args.ntrain}_{args.split_ix}"
    return sfx
...
def main():
    ...
    if not args.out_dir:
        args.out_dir = os.path.join(os.path.realpath(args.emb_dir), "results_mclr/")
    ...
    x_train, y_train = load_feats_and_labels(
        emb_dir, args.in_lang, f"train_{args.ntrain}", args
    )
...
def parse_arguments():
    ...
    parser.add_argument("emb_dir", default="", help="path to dir with embeddings")
