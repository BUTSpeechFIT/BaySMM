...
"""
Train a classifier on embeddings from source language and test on all the languages.
"""
...
def train_classifier(feats, labels, args):
    ...
    if args.clf != "glc":
        clf_pkl = os.path.join(
            out_dir,
            f"{args.clf}-{args.trn}_{args.in_lang}_{args.ntrain}_{args.split_ix}.pkl",
        )
    else:
        clf_pkl = os.path.join(
            out_dir, f"{args.clf}_{args.in_lang}_{args.ntrain}_{args.split_ix}.pkl"
        )
...
def classify_test_data(clf, emb_dir, config, langs, args, scores_dict, proba_dict):
    ...
    for i, set_name in enumerate(["dev", "test"]):
        ...
        for j, lang in enumerate(langs):
            ...
            if feats_fname:
                labels_fname = get_labels_fname(lang, set_name, args.labels_dir, args.split_ix)
            else:
                continue

            if args.clf in ("glcu", "glcu-0"):
                feats = rcv_utils.load_feats(feats_fname, dim="full")
            else:
                feats = rcv_utils.load_feats(feats_fname, dim="half")

            labs = rcv_utils.load_labels(labels_fname, L2I)

            if labs.shape[0] == feats.shape[1]:
                feats = feats.T

            rcv_utils.check_feats_and_labels(feats, labs)
...
def main():
    ...
    if not args.out_dir:
        args.out_dir = os.path.realpath(args.emb_dir) + "/../results/"
    ...
    train_feats_file = get_feat_fname(
        emb_dir, args.in_lang, config, f"train_{args.ntrain}", args
    )
    train_labels_file = get_labels_fname(
        args.in_lang, f"train_{args.ntrain}", args.labels_dir, args.split_ix
    )

    if args.verbose:
        print("\nLoading training embeddings:", train_feats_file)

    if args.clf in ("glcu", "glcu-0"):
        feats = rcv_utils.load_feats(train_feats_file, dim="full")
    else:
        feats = rcv_utils.load_feats(train_feats_file, dim="half")

    # Label name to integer mapping
    labels = rcv_utils.load_labels(train_labels_file, L2I)

    if labels.shape[0] == feats.shape[1]:
        feats = feats.T

    rcv_utils.check_feats_and_labels(feats, labels)
    ...
    print("Scores, pred proba saved in", args.out_dir)
...
def parse_arguments():
    ...
    parser.add_argument("labels_dir", help="path to dir with labels (random_data_splits/)")
